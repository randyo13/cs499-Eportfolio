...
public:
    ...
    // ENHANCEMENT: ----------------------------------------------------------------------------------  Refactored score calculation into two helper functions for modularity (Triplets and Singles)
    int scoreTriplets(std::vector<int>& counts);                // Calculate Triplets (three of a kind)
    int scoreSingles(const std::vector<int>& counts);           // Calculate single 1s and 5s
